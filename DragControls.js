...
class DragControls extends EventDispatcher {
	...
	constructor( _objects,_scene,_actions, _camera, _domElement ) {
			...
			if ( _selected ) {
				if ( _raycaster.ray.intersectPlane( _plane, _intersection ) ) {
					if(_selected.userData.isDraggable){
						...
						if(_selected.position.x>95){
							_selected.position.x=95;
						}
						if(_selected.position.x<-73){
							_selected.position.x=-73;
						}
						if(_selected.position.y<-1.5 && _selected.position.x<0 &&_selected.position.x>-40)
							_selected.position.y=-1.5;
						// Logic.init(_selected,_objects,_actions);
						if(_selected.name!='KETTLE'){
							// console.log(_selected.name);
							// console.log(_selected.position);
							if(_selected.position.x<30 &&_selected.position.x>-25 && _selected.position.y<8){
						...
						if(_selected.name=='KETTLE'){
							// console.log(_selected.position);
							if(_selected.position.x<34 && _selected.position.x>0 && _selected.position.y<10){
		...
		function onPointerCancel() {
			...
			// 	if(_selected.name=='TEAPOT'){
			// 		flag=1;
			// 		Logic.init(_selected);
			// 	}
			// 	else if(_selected.name=='COFFEE' && flag==1){
			// 		flag=2;
			// 		Logic.init(_selected);
			// 	}
			// 	else if(_selected.name=='MILK' && flag==2){
			// 	 	flag=3;
			// 		 Logic.init(_selected);
			// 	}
			// 	else if (_selected.name=='SUGAR' && flag==3){
			// 		flag=4;
			// 		Logic.init(_selected);
			// 	}
